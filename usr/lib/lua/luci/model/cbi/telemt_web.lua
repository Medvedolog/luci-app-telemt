...
-- -----------------------------------------------------------------------------
-- Frontend helper state and explicit actions.
-- The action buttons are XHR requests to a separate controller endpoint, not CBI
-- submit buttons. Therefore they only use already committed UCI state and cannot
-- accidentally apply unsaved values from this form.
-- -----------------------------------------------------------------------------
frontst = m:section(SimpleSection)
frontst.title = _("HAProxy helper")
frontst.description = _("Save & Apply the WEB configuration first. Check, Apply and Restore below operate only on the configuration already committed to /etc/config/telemt.")
...
<div style="display:flex;gap:8px;flex-wrap:wrap;margin:4px 0 8px 0">
  ...
  <button type="button" class="cbi-button cbi-button-apply" onclick="return telemtWebFrontendAction('apply', true)">Apply HAProxy</button>
  <button type="button" class="cbi-button cbi-button-reset" onclick="return telemtWebFrontendAction('restore', true)">Restore previous HAProxy</button>
</div>
...
function telemtWebFrontendAction(act, destructive) {
    if (destructive) {
        var msg = (act === 'apply')
            ? 'Apply the Telemt-managed HAProxy configuration using SAVED UCI settings?'
            : 'Restore the HAProxy configuration saved before Telemt took control?';
